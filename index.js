...
class CustomBlockManager {
  ...
  static load(BlockConstructor) {
    ...
    this.plugin.eventBus.on("click-blockicon", (e) => {
      if (
        e.detail.blockElements[0].getAttribute("custom-plugin-id") ===
        this.plugin.name
      ) {
  ...
  static build(type, data) {
    ...
    const ial = `{: custom-plugin-id="${this.plugin.name}" id="${id}"}`;
        ...
        const ob = new MutationObserver((events) => {
          for (const e of events) {
            if (e.removedNodes.length) {
              ...
              this.blocks.delete(id);
              block.onDestroyed && block.onDestroyed();
            }
          }
        });
